...
GLAMBDA:CreateConVar( "player_respawn_time", "3.0", "The time the player will respawn after being killed.", nil, nil, nil, 0, 60, {
    name = "Respawn Time",
    category = "Server Settings",
} )
...
GLAMBDA:CreateConVar( "voice_norespawn", false, "If the players that are currently speaking can't respawn until they finish their speech.", {
...
GLAMBDA:CreateConCommand( "cmd_update_data", function( ply )
    ...
    if plyValid and !ply:IsSuperAdmin() then return end
...
GLAMBDA:CreateConCommand( "cmd_transfer_lambdadata", function( ply )
    ...
    print( "GLambda Players: Transfered Lambda data files via console command. Ran by ", ( plyValid and ply:Name() .. " | " .. ply:SteamID() or "Console" ) )

end, false, "Transfers and copies the data files from Lambda Players to GLambda Players.\nMake sure to update the data once you change this to actually load in the files!", { 
    name = "Transfer Lambda Data", 
    category = "Utilities" 
} )